...
echo '<div style="float:right; font-weight:800; font-size:16px; "><a href="rpt_banq_sale.php"><strong>Back</strong></a></div>';
echo '<div width="1300">
...
while ($from1 < $to) {
	...
	if ($result1_fetch[0] != "") {
		while ($i < $num) {
			...
			$morder_no = $row1["ORDER_NO"];
			$banq_id = $row1["BANQ_ID"];
			$function_id = $row1["FUNCTION_TYPE_ID"];
			$function_date = $row1["FUNCTION_DATE"];
			$host_name = $row1["GUEST_NAME"];
			$contact_no = $row1["CONTACT_NO"];
			$total = $row1["grand_total"];
			$cash = $row1["total_received"];
			$credit = $total - $cash;
			...
			if ($morder_no != 0) {
				...
				if ($result3_fetch[2] != 0) {
					$CR_NO = $result3_fetch["CR_NO"];
					$INSTALMENT_AMOUNT = $result3_fetch["INSTALMENT_AMOUNT"];
					$CREATED_ON = $result3_fetch["CREATED_ON"];
				}
			}
			$j1 = $j1 + 1;
