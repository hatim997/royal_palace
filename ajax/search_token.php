...
<?php
	...
	?>
    <form name="form1" method="post" action="pool_token.php">
    <table width="1005" border="0" >
	<?php
if($type == "id")
	{
		
$sql="SELECT * FROM pool_payment_token WHERE TOKEN_ID = '$value'";
  	...
  	{  ?>
			...
			echo "<td>" . $row['TOKEN_ID'] ."</td>";
			 echo "<td>" . $row['MEMBERSHIP_ID'] ."</td>";
				...
				$b_id=$row['SWIMMER_TYPE_ID'];
			 $sql1="SELECT * FROM SWIMMER_TYPE_TAB WHERE SWIMMER_TYPE_ID = '$b_id'";
			 $result1= mysql_query($sql1);
			 while($row1=mysql_fetch_array($result1))
			 {
				echo "<td>" . $row1['SWIMMER_TYPE_TITLE'] . "</td>";
			 }
			 	...
			 	$c_id=$row['CHARGES_ID'];
			 $sql2="SELECT * FROM pool_charges_plan WHERE CHARGES_ID = '$c_id'";
			 $result2= mysql_query($sql2);
			 while($row2=mysql_fetch_array($result2))
			 {
				echo "<td>" . $row2['SWIMMING_CHARGES'] . "</td>";
			 }
...
if($type == "m_id")
	{
$sql="SELECT * FROM pool_payment_token WHERE MEMBERSHIP_ID ='$value'";
$result = mysql_query($sql);
$count=mysql_num_rows($result);
if($count != "")
		 {
