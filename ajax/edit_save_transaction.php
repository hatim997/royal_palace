...
<?php
	...
	while($i < $num1)
	{
		...
		$old_amount = mysql_result($result1,$i, "TRN_AMOUNT" );
		$old_amounttype = mysql_result($result1,$i, "AMOUNT_TYPE" );
	...
	if($old_amounttype == "DB")
	{
		$query =  "UPDATE voucher_master SET TOTAL_DEBIT = TOTAL_DEBIT - '$old_amount' 
					WHERE VOUCHER_ID = '$voucherid' AND CREATED_ON LIKE '$current_date%'";
		mysql_query($query) or die("Failed 1: ".mysql_error());
		if($new_amounttype == "DB")
		{
			$query =  "UPDATE voucher_master SET TOTAL_DEBIT = TOTAL_DEBIT + '$new_amount' 
						WHERE VOUCHER_ID = '$voucherid' AND CREATED_ON LIKE '$current_date%'";
			mysql_query($query) or die("Failed 1: ".mysql_error());
		}
		else
		{
			...
			$query =  "UPDATE voucher_master SET TOTAL_CREDIT = TOTAL_CREDIT + '$new_amount' 
						WHERE VOUCHER_ID = '$voucherid' AND CREATED_ON LIKE '$current_date%'";
	...
	{
		$query =  "UPDATE voucher_master SET TOTAL_CREDIT = TOTAL_CREDIT - '$old_amount' 
					WHERE VOUCHER_ID = '$voucherid' AND CREATED_ON LIKE '$current_date%'";
		mysql_query($query) or die("Failed 2: ".mysql_error());
		if($new_amounttype == "DB")
		{
	...
	######## TOTAL DEBIT/ TOTAL CREDIT UPDATION CODE ########
	$query =  "UPDATE voucher_transaction SET TRN_AMOUNT = '$new_amount', AMOUNT_TYPE = '$new_amounttype', TRN_NARRATION = '$new_detail' WHERE TRANSACTION_ID = '$transactionid'";
	mysql_query($query) or die("Failed 1: ".mysql_error());
