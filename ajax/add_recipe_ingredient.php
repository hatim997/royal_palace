...
WHERE store_master_tab.INGREDIENT_ID = '$ing_name'";
	...
	while ($i < $num1) {
		...
		$low_unit = $row["LOW_UNIT"];
		$div_value = $row["DIVISION_VALUE"];
		$i++;
...
WHERE dish_master_tab.DISH_ID = '$dishid'";
	...
	while ($i < $num1) {
		$row = mysqli_fetch_assoc($result1);
		$food_type = $row['FOOD_TYPE_DETAIL'];
	...
	$result2 = mysqli_query($conn, $query2) or die("Failed: " . mysqli_error($conn));
	$num2 = mysqli_num_rows($result2);
	$i = 0;
	while ($i < $num2) {
		$row = mysqli_fetch_assoc($result2);
		$dish_name = $row["DISH_NAME"];               // removed table prefix, it's unnecessary here
		$cooking_time = $row["COOKING_TIME"];
		$sale_price = $row["DISH_CHARGES"];           // assuming joined table fields are selected
		$i++;
	}
	...
	while ($i < $num1) {
		...
		$cost_id = $row1["COST_ID"];
		$energy = $row1["ENERGY_COST"];
		$hr = $row1["HR_COST"];
		$other = $row1["OTHER_COST"];
		$i++;
	}
	// while loop ends her
		...
		<table align="left" width="244">
			<tr>
				...
				<td width="147" align="left"><?php echo $dish_name; ?>
					<input type="hidden" id="dishid" name="dishid" value="<?php echo $dishid; ?>" />
				</td>
			</tr>
			<tr>
				...
				<td width="146" align="left">
					<select style="width:145px;" id="ing_name" name="ing_name">
						...
						while ($i < $num1) {
							...
							// $unit_id = $row["UNIT_ID"]; // You can use this if needed
							echo '<option value="' . $ingredient_id . '">' . htmlspecialchars($ingredient_name) . '</option>';
							$i++;
						}
						?>
					</select>
	...
	<div style="overflow:auto; height:530px; width:1000px; float:right; background-color:#DCECEF;" align="right" id="center">
		...
		<div style="overflow:auto; height:auto; width:1000px;" id="recipe">
			...
			while ($i < $num1) {
				$j = $i + 1;
				$row = mysqli_fetch_assoc($result1);  // fetch associative array
				...
				echo '<a href="javascript:showPopup(' . $ingredient_code . ')" alt="Edit" title="Edit"><img width="25" height="25" SRC="images/edit_ing.png" alt="Edit"/>';
				echo "</td>";
				echo "<td width='70px' align='center'>";
				echo '<a href="javascript:delete_ingredient(' . $dishid . ',' . $ingredient_code . ',' . $required_quantity . ',' . $divval . ')" alt="Delete" title="Delete"><img width="25" height="25" SRC="images/remove.png" alt="Delete"/>';
				echo "</td>";
				echo "</tr>";
