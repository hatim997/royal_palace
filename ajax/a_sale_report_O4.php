...
<?php
	...
	echo '<div style="float:right;"><a href="banq_sale_report.php"><strong>Back</strong></a></div>';
	echo '<div width="1350">
	  <table width="1350" border="0" cellpadding="0" cellspacing="0" align="left" style="border:1px solid #8e3f2b;">
		<thead>
		<tr style="background: #8e3f2b repeat-x top left; background-position:center; color:#FFFFFF; font-size:16px; height:30px">
				...
				<td width="80" align="center"><strong>Booking<br>Date</td>
				<td width="80" align="center"><strong>Payment<br>Date</strong></td>
				<td width="80" align="center"><strong>Order No</strong></td>
				<td width="80" align="center"><strong>Banquet<br>Name</strong></td>
				<td width="80" align="center"><strong>Function<br>Type</strong></td>
				<td width="80" align="center"><strong>Function<br>Date</strong></td>
				<td width="80" align="center"><strong>Guest<br>Name</strong></td>
				<td width="80" align="center"><strong>Contact<br>No</strong></td>
				<td width="80" align="center"><strong>Total<br>Payable</strong></td>
				<td width="50" align="center"><strong>CR-No</strong></td>
				<td width="80" align="center"><strong>Received<br>Amount</strong></td>
				<td width="80" align="center"><strong>To-Date<br>Received</strong></td>
	...
	while($from1 < $to)
	{
		...
		$query = "SELECT * FROM banq_order_master WHERE BOOKING_DATE >= '$start' AND BOOKING_DATE <= '$end' 
		ORDER BY ORDER_NO ASC";
		$result1 = mysql_query($query) or die ('Banquet failed!'.mysql_error());
		$result1_fetch = mysql_fetch_array($result1);
		
		$num = mysql_num_rows($result1);
		$i=0;
		//<input name="head_id_old" type="hidden" id="head_id_old" size=25 value="echo $result1_fetch[0]">;
		//$morder_no = $_POST('$result1_fetch[0]');
		$morder_no =('$result1_fetch[0]');
		//echo $morder_no ;
		...
		
		{
			...
			//$i++;
			//}
			$morder_no = mysql_result($result1,$i, "ORDER_NO" );
			$banq_id = mysql_result($result1,$i, "BANQ_ID");
			$function_id = mysql_result($result1,$i, "FUNCTION_TYPE_ID" );
			$function_date = mysql_result($result1,$i, "FUNCTION_DATE" );
			$host_name = mysql_result($result1,$i, "GUEST_NAME" );
			$contact_no = mysql_result($result1,$i, "CONTACT_NO" );
			...
			$result2 = mysql_query($query2) or die ('Banquet failed!'.mysql_error());
			$BANQ_NAME = mysql_result($result2,0, "BANQ_NAME" );
			}
			
			if($function_id!=0)
			{
			$query4 = "SELECT FUNCTION_TYPE_NAME FROM function_type_tab WHERE FUNCTION_TYPE_ID = '$function_id'";
			$result4 = mysql_query($query4) or die ('Banquet failed!'.mysql_error());
			$FUNCTION_TYPE_NAME = mysql_result($result4,0, "FUNCTION_TYPE_NAME" );
			}
			
			if($morder_no!=0)
			//if ('$result1_fetch[0]' !=""); //order no
			{
			$query3 = "SELECT INSTALMENT_AMOUNT, CR_NO, CREATED_ON FROM payment_serial
			WHERE ORDER_ID = '$morder_no'";
			$result3 = mysql_query($query3) or die ('Banquet failed!'.mysql_error());
			$num3 = mysql_num_rows($result3);
			
			if ($result3!="")
			{
			$CR_NO = mysql_result($result3,0 ,"CR_NO");
			$INSTALMENT_AMOUNT = mysql_result($result3,0, "INSTALMENT_AMOUNT" );
			$CREATED_ON = mysql_result($result3,0, "CREATED_ON" );
			}
			}
			$j1 = $j1 + 1;
		
			?>
            <tr style="background-color:<?php if($d%2 == 0) echo "#9C9"; else echo "#999";?>; height:30px; font-size:14px;">
...
?>
    ...
    <table width="1350" border="0" cellpadding="0" cellspacing="0" align="left">
        <tr style="height:30px; font-size:14px;">
            ...
            <td width="80" align="center"><?php echo $banq_total; ?></td>
            <td width="50" align="center">&nbsp;</td>
            <td width="80" align="center">&nbsp;</td>
            <td width="80" align="center"><?php echo $banq_cash; ?></td>
