...
echo '<div style="margin-left:750px; font-weight:800; font-size:16px; "><a href="rpt_banq_collection_by_function.php"><strong>Refresh</strong></a>
</div>';
...
//</div';
	...
	//	$query = "SELECT * FROM banq_order_master WHERE FUNCTION_DATE >= '$start' AND FUNCTION_DATE <= '$end' AND BOOKING_STATUS='B' ORDER BY ORDER_NO ASC";
		$query = "SELECT * FROM banq_order_master WHERE FUNCTION_DATE>='$start' AND FUNCTION_DATE<='$end' ORDER BY ORDER_NO ASC";
		...
		while($i < $num)
		{
			...
			?>
            <tr style="background-color:<?php if($d%2 == 0) echo "#9C9"; else echo "#2CC";?>; height:30px; font-size:14px;">
