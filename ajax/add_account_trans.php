...
<body>
	...
	$query = "SELECT * FROM voucher_master where VOUCHER_ID = '$voucherid' AND CREATED_ON LIKE '$current_date%'";
	$result = mysqli_query($conn, $query);
	if (mysqli_num_rows($result) > 0) {
		if ($amounttype == "DB") {
			$query =  "UPDATE voucher_master SET TOTAL_DEBIT = TOTAL_DEBIT + '$trn_amount' 
		  WHERE VOUCHER_ID = '$voucherid' AND CREATED_ON LIKE '$current_date%'";
			mysqli_query($conn, $query) or die("Failed 1: " . mysqli_error($conn));
		} else if ($amounttype == "CR") {
			$trn_amount = $trn_amount * -1;
			$query =  "UPDATE voucher_master SET TOTAL_CREDIT = TOTAL_CREDIT + '$trn_amount' 
		  WHERE VOUCHER_ID = '$voucherid' AND CREATED_ON LIKE '$current_date%'";
			mysqli_query($conn, $query) or die("Failed 2: " . mysqli_error($conn));
		}
	} else {
		if ($amounttype == "DB") {
	...
	while ($i < $num1) {
		...
		$account_title = mysqli_fetch_assoc($result1)["SCAT2_ACCT_TITLE"];
		$i++;
	}
	...
	while ($i < $num1) {
		mysqli_data_seek($result1, $i);
		$trans_id = mysqli_fetch_assoc($result1)["TRANSACTION_ID"];
		$i++;
	}
	$trans_id++;
	...
	VALUES('$voucherid', '$trans_id', '$vouchertype' , '$current_date', '$accountcode', '$account_title', '$trn_date', '$trn_amount',
	...
	while ($i < $num1) {
		mysqli_data_seek($result1, $i);
		$vtype = mysqli_fetch_assoc($result1)["VOUCHER_TITLE"];
		$i++;
	}

	$query1 = "SELECT * FROM voucher_master WHERE VOUCHER_ID = '$voucherid'  AND CREATED_ON LIKE '$current_date%'";
	$result1 = mysqli_query($conn, $query1) or die("Failed :" . mysqli_error($conn));
	$num1 = mysqli_num_rows($result1);
	$i = 0;
	while ($i < $num1) {
	...
	while ($i < $num1) {
		mysqli_data_seek($result1, $i);
		$last_date = mysqli_fetch_assoc($result1)["LAST_POSTING_DATE"];
		$i++;
	}
		...
		<table align="left" width="998">
			...
			<tr>
				...
				<td width="441" height="25" align="center">
				  ...
				  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
				  <a href="voucher_print.php?vid=' . $voucherid . '&vdate=' . $current_date . '" alt="Print" title="Print">
	...
	<div style="border: 1px solid black; overflow:auto; height:280px; width:1000px;" align="center" id="center">
		<table width="1000" align="left" class="altrowstable" id="alternatecolor">
			...
			$query1 = "SELECT * FROM voucher_transaction WHERE VOUCHER_ID = '$voucherid' AND VOUCHER_DATE = '$current_date'";
			$result1 = mysqli_query($conn, $query1) or die("Failed :" . mysqli_error($conn));
			$num1 = mysqli_num_rows($result1);
			$i = 0;
			$j = 0;
			while ($i < $num1) {
				...
				$trn_amount     = $row["TRN_AMOUNT"];
				$trn_detail     = $row["TRN_NARRATION"];
				$j = $i + 1;
				$debit = 0;
				$credit = 0;
				if ($type == "DB") {
		...
		<table width="986" align="left">
			...
			<tr>
				...
				<td valign="top" style="padding-top:10px;"><select style="width:145px;" id="title" name="title" onchange="voucher_title(this.value);">
						...
						<?php
						$conn = mysqli_connect("localhost", "username", "password", "database"); // replace with your actual credentials
						...
						while ($i < $num1) {
							...
							$main_acct_code = $row["MAIN_ACCT_CODE"];
							$ctrl_acct_code = $row["CTRL_ACCT_CODE"];
							$scat1_acct_code = $row["SCAT1_ACCT_CODE"];
							$scat2_acct_code = $row["SCAT2_ACCT_CODE"];
							$scat2_acct_title = $row["SCAT2_ACCT_TITLE"];
