...
if (isset($_REQUEST['submit'])) {
	$query0 = "INSERT INTO pool_payment_token(
		TOKEN_ID, MEMBERSHIP_ID, SWIMMER_TYPE_ID, CHARGES_ID, CHARGES_TYPE_ID, CHARGES_DUE, 
		CHARGES_FLAG, PAYMENT_MODE, CREDIT_CARD_NO, BANK_NAME, CHEQUE_NO, CREATED_ID, CREATED_ON
	) VALUES (
		'$mem_id', '$mem_id2', '$swim_type2', '$charge_id', '$charge_type_id', '$due', '$charge_flag',
		'$p_mode', '$card_no', '$bank', '$chequeno', '$created_id', '$created_on'
	)";
	$result0 = mysqli_query($conn, $query0) or die("error" . mysqli_error($conn));
} else if (isset($_REQUEST['update'])) {
	$query0 = "UPDATE pool_payment_token SET 
		TOKEN_ID = '$mem_id',
		MEMBERSHIP_ID = '$mem_id2',
		SWIMMER_TYPE_ID = '$swim_type2',
		CHARGES_ID = '$charge_id',
		CHARGES_TYPE_ID = '$charge_type_id',
		CHARGES_DUE = '$due',
		CHARGES_FLAG = '$charge_flag',
		PAYMENT_MODE = '$p_mode',
		CREDIT_CARD_NO = '$card_no',
		BANK_NAME = '$bank',
		CHEQUE_NO = '$chequeno',
		EDITED_ID = '$created_id',
		EDITED_ON = '$created_on' 
		WHERE TOKEN_ID = '$mem_id'";
	$result0 = mysqli_query($conn, $query0) or die("error" . mysqli_error($conn));
}
...
$query = "SELECT * FROM SWIMMER_TYPE_TAB WHERE SWIMMER_TYPE_ID = '$swim_type2'";
$result = mysqli_query($conn, $query) or die('Not Found: ' . mysqli_error($conn));
$row = mysqli_fetch_array($result);

$swim_type_det = $row['SWIMMER_TYPE_TITLE'];
	...
	<div id="c_content">
		<div id="mydiv">
			...
			<table width="392" height="179" frame="box" background="images/watermark.png">
				...
				<tr>
					<th width="77">
						<div align="justify">Swimmer Type</div>
					</th>
					<td width="178">
		...
		<input name="exit" type="button" value="Exit" style="background:#91400F; height:30px; width:115px; font-weight:700;" onclick="window.location.href='pool_token.php'" />
	</div>
</body>
