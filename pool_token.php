...
if (isset($_REQUEST['view'])) {
	foreach ($_REQUEST['radiobox'] as $t_id) {
		$sql1 = "SELECT * FROM pool_payment_token WHERE TOKEN_ID = '$t_id'";
...
<head>
	...
	<script type="text/javascript">
		function payment_mode(value) {
			...
			xmlhttp.open("POST", "ajax/payment_mode_pool.php", true);
	...
	<script type="text/javascript">
		function validateForm() {
			...
			if ((document.getElementById("mem_id2").value && document.getElementById("swim_type2").value && document.getElementById("c_status").value) == "") {
	...
	<script type="text/javascript">
		function search_pool_token() {
			...
			xmlhttp.open("POST", "search_pool_token.php", true);
			xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
			xmlhttp.send();
		}
	</script>
	<script type="text/javascript">
		function search_token() {
			...
			xmlhttp.open("POST", "ajax/search_token.php", true);
			xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
...
<body id="main_body">
	...
	<div id="form_container">
		...
		<?php
		$query2 = "SELECT * FROM pool_payment_token WHERE TOKEN_ID = '$t_num'";
		$result2 = mysqli_query($conn, $query2) or die('Query Failed00');
		if (mysqli_num_rows($result2) == 0) {
		?>

			<form id="form1" class="appnitro" enctype="multipart/form-data" method="post" action="pool_token_print.php" onsubmit="return validateForm(); ">
				...
				<?php
				$query33 = "SELECT MAX(TOKEN_ID) AS max_token FROM pool_payment_token";
				$result33 = mysqli_query($conn, $query33);
				$row33 = mysqli_fetch_array($result33);
				$row33 = $row33['max_token'];

				$query3 = "SELECT MAX(MEMBERSHIP_ID) AS max_member FROM pool_payment_token";
				$result3 = mysqli_query($conn, $query3);
				$row3 = mysqli_fetch_array($result3);
				$row3 = $row3['max_member'];
				...
				<table>
					<tr>
						<td width="113"><label class="description" for="element_46">Swimmer Type* </label></td>
	...
	WHERE CHARGES_ON_DATE <= '$dd' AND CHARGES_OFF_DATE >= '$dd' AND CHARGES_TYPE_ID = '2'";
				...
				<table width="945">
					<tr>
						...
						<td width="287"><input id="charge" name="charge" type="text" value="<?php echo $row4['SWIMMING_CHARGES']; ?>" />
				...
				<table>
					<tr>
						...
						<td width="116"><input name="search" type="button" value="Search" style="background:#91400F; height:30px; width:115px; font-weight:700;" onclick="search_pool_token();" /></td>
		...
		?>
			...
			<form id="form1" class="appnitro" method="post" action="pool_token_print.php" onsubmit="return validateForm(); ">
				<div class="form_description">
					<h2>Swimming Pool</h2>
					<p>Token Form</p>
				</div>
					...
					<tr>
						...
						<td width="179"><select id="swim_type2" name="swim_type2" onchange="generate_id(this.value);">
								...
								while ($i < $num) {
									...
									echo '<option value="' . $swimmerid . '" selected="selected">' . $swimmer_detail . '</option>';
									$i++;
								}

								// Other swimmer types
								$query1 = "SELECT * FROM SWIMMER_TYPE_TAB WHERE SWIMMER_TYPE_ID != '$s_id'";
								$result1 = mysqli_query($conn, $query1) or die('Not Found: ' . mysqli_error($conn));
								$num1 = mysqli_num_rows($result1);
								$i1 = 0;
								while ($i1 < $num1) {
									mysqli_data_seek($result1, $i1);
									$row_swim1 = mysqli_fetch_array($result1);
									$swimmerid1 = $row_swim1['SWIMMER_TYPE_ID'];
									$swimmer_detail1 = $row_swim1['SWIMMER_TYPE_TITLE'];
	...
	WHERE CHARGES_ID = '$dd'";
				...
				<table>
					<tr>
						...
						<td><input name="new_member" type="button" value="New Token" style="background:#91400F; height:30px; width:115px; font-weight:700;" onclick="window.location.href='pool_token.php'" /></td>
