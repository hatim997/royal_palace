...
if (isset($_REQUEST['add'])) {
	...
	$query0 = "INSERT INTO pool_charges_plan (CHARGES_ID, CHARGES_TYPE_ID, SWIMMING_CHARGES, 
	...
	$query0 = "UPDATE pool_charges_plan SET CHARGES_ID = '$charge_id', CHARGES_TYPE_ID = '$charge_type', SWIMMING_CHARGES = '$s_charge',
    CHARGES_ON_DATE = '$f_date', CHARGES_OFF_DATE = '$t_date', EDITED_ID = '$edited_id',
    EDITED_ON = '$edited_on' WHERE CHARGES_ID = '$charge_id'";
	$result0 = mysqli_query($conn, $query0) or die("error" . mysqli_error($conn));

	$sql1 = "SELECT * FROM pool_charges_plan WHERE CHARGES_ID = '$charge_id'";
	...
	$query0 = "DELETE FROM pool_charges_plan WHERE CHARGES_ID = '$charge_id'";
	$result0 = mysqli_query($conn, $query0) or die("error" . mysqli_error($conn));
	...
	foreach ($_REQUEST['radiobox'] as $c_id) {
		$sql1 = "SELECT * FROM pool_charges_plan WHERE CHARGES_ID = '$c_id'";
		$result1 = mysqli_query($conn, $sql1) or die("error" . mysqli_error($conn));
	}
	$row = mysqli_fetch_array($result1);
...
<head>
	...
	<script type="text/javascript">
		function search_pool_charge() {
			...
			xmlhttp.open("POST", "search_pool_charge.php", true);
			xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
			xmlhttp.send();
		}
	</script>
	<script type="text/javascript">
		function search_cplan_value() {
			...
			xmlhttp.open("POST", "ajax/search_cplan_value.php", true);
			xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
			xmlhttp.send("type=" + type);
		}
	</script>
	<script type="text/javascript">
		function search_pool_charge_plan() {
			...
			xmlhttp.open("POST", "ajax/search_pool_charge1.php", true);
			xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
...
<body id="main_body">
	...
	<div id="form_container">
		...
		<form id="form1" class="appnitro" method="post" action="" onsubmit="return validateForm(); ">
			...
			<?php
			$query2 = "SELECT * FROM pool_charges_plan WHERE CHARGES_ID = '$m_num'";
			$result2 = mysqli_query($conn, $query2) or die('Query Failed00');

			if (mysqli_num_rows($result2) == 0) {
				$query3 = "SELECT MAX(CHARGES_ID) AS max_id FROM pool_charges_plan";
				$result3 = mysqli_query($conn, $query3);
				$row3 = mysqli_fetch_array($result3);
				$row3_max = $row3['max_id'];
			?>
				...
				<table width="945">
					...
					<tr>
						...
						<td width="328"> <select id="charge_type" name="charge_type">
								...
								<?php
								$query1 = "SELECT * FROM pool_charges_type_tab";
				...
				<table>
					<tr>
						...
						<td width="116"><input name="search" type="button" value="Search" style="background:#91400F; height:30px; width:115px; font-weight:700;" onclick="search_pool_charge();" /></td>
				...
				<table width="945">
					...
					<tr>
						...
						<td width="328"> <select id="charge_type" name="charge_type">
								...
								$query11 = "SELECT * FROM pool_charges_type_tab WHERE CHARGES_TYPE_ID = '$ii'";
								...
								$query1 = "SELECT * FROM pool_charges_type_tab WHERE CHARGES_TYPE_ID != '$ii'";
								$result1 = mysqli_query($conn, $query1) or die('Not Found: ' . mysqli_error($conn));
								$num1 = mysqli_num_rows($result1);
								$i1 = 0;
								while ($i1 < $num1) {
					...
					<tr>
						<td width="105"><label class="description" for="element_19">Charges </label></td>
						<td width="358"> <input id="s_charge" name="s_charge" type="text" value="<?php echo $row['SWIMMING_CHARGES']; ?>" /> </td>
					</tr>
				</table>
				<div id="mode"></div>
				<li class="section_break"></li>
