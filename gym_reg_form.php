...
if (isset($_REQUEST['view'])) {
	foreach ($_REQUEST['radiobox'] as $m_id) {
		$sql1 = "SELECT * FROM gym_membership_master_tab WHERE MEMBERSHIP_ID ='$m_id'";
...
//submit the membership form
	...
	if ((($_FILES["image_path"]["type"] == "image/gif")
		...
		if ($_FILES["image_path"]["error"] > 0) {
			...
			$target = "images/gym_mem/" . $_FILES["image_path"]["name"];
	...
	$query0 = "INSERT INTO gym_membership_master_tab(
    MEMBERSHIP_ID, GYM_TYPE_ID, FIRST_NAME, LAST_NAME, FATHER_NAME,
    HUSBAND_NAME, DATE_OF_BIRTH, AGE_YEAR, GENDER, SVC_ID, CNIC_NO, ADDRESS, CITY, COUNTRY, COMPANY_NAME,
    OFFICE_FIXED_LINE, RESID_FIXED_LINE, MOBILE_NO, EMERGENCY_MOBILE, GYM_FLAG, MEMBER_APPLIED_ON, MEMBER_PERIOD_ID, 
    ...
    '$mem_id','$gym_type', '$f_name', '$l_name', '$father_name',
    '$husband_name', '$dob', '$age', '$gender', '$svc_id', '$CNIC', '$address', '$city', '$country', '$c_name',
    '$contact_no_off', '$contact_no_res', '$mob_no', '$emergency_no', '$gymer', '$mem_date', '$mem_type',
	...
	$sql1 = "SELECT * FROM gym_membership_master_tab WHERE MEMBERSHIP_ID = '$mem_id'";
	$result1 = mysqli_query($conn, $sql1) or die("error: " . mysqli_error($conn));
	$row = mysqli_fetch_array($result1);
...
//updation of record
	...
	$query0 = "UPDATE gym_membership_master_tab SET 
    MEMBERSHIP_ID = '$mem_id', 
    GYM_TYPE_ID = '$gym_type', 
    FIRST_NAME = '$f_name',
    LAST_NAME = '$l_name', 
...
<head>
	...
	<script type="text/javascript">
		function search_gym_member() {
			...
			xmlhttp.open("POST", "search_gym_member.php", true);
	...
	<script type="text/javascript">
		function search_member_gym() {
			...
			xmlhttp.open("POST", "ajax/search_member_gym.php", true);
			xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
	...
	<script type="text/javascript">
		function validateForm() {
			...
			if ((document.getElementById("mem_type").value && document.getElementById("gym_type").value && document.getElementById("f_date").value &&
					document.getElementById("t_date").value && document.getElementById("f_name").value && document.getElementById("l_name").value &&
					document.getElementById("CNIC").value && document.getElementById("mob_no").value && document.getElementById("gymer").value &&
	...
	<script>
		function submitMe(obj) {
			if (obj.value == "Print") {
				document.getElementById('form1').action = 'gym_print_reg_form.php'
			}
...
<body id="main_body">
	...
	<div id="form_container">
		...
		<?php
		$query2 = "SELECT * FROM gym_membership_master_tab WHERE MEMBERSHIP_ID = '$m_num'";
		...
		if (mysqli_num_rows($result2) == 0) { ?>
			...
			<form id="form1" class="appnitro" enctype="multipart/form-data" method="post" action="" onsubmit="return validateForm(); ">
				...
				<?php
				$query3 = "SELECT MAX(MEMBERSHIP_ID) FROM gym_membership_master_tab";
				...
				<table width="945">
					...
					<tr>
						...
						<td width="103"><label class="description" for="element_31">Gym Type* </label></td>

						<td width="144">

							<select id="gym_type" name="gym_type">
								...
								while ($i < $num) {
									...
									echo '<option value="' . $gymid . '">' . $gym_detail . '</option>';
									$i++;
								}
								?>
							</select>
					...
					<tr>
						...
						<td>
							<select id="mem_type" name="mem_type">
								<option value="">Select Type</option>
								<?php
								$query1 = "SELECT * FROM gym_membership_period_tab";
				...
				<table width="945">
					<tr>
						<td width="187"><label class="description" for="element_26">Gym Flag* </label></td>
						<td width="164">
							<select id="gymer" name="gymer">
           ...
           WHERE CHARGES_ON_DATE <= '$dd' AND CHARGES_OFF_DATE >= '$dd' AND CHARGES_TYPE_ID = '1'";
						...
						<td width="164">
							<input id="mem_fee" name="mem_fee" type="text" value="<?php echo $row4['GYM_CHARGES']; ?>" />
				...
				<table>
					<tr>
						...
						<td width="116"><input name="search" type="button" value="Search" style="background:#91400F; height:30px; width:115px; font-weight:700;" onclick="search_gym_member();" /></td>
		...
		?>
			<!--for update-->
			<form id="form1" class="appnitro" enctype="multipart/form-data" method="post" action="" onsubmit="validateForm();">
				<div class="form_description">
					<h2>Gym</h2>
					<p>Member Registration form</p>
				</div>
						...
						<td width="218">
							...
							<select id="gym_type" name="gym_type">
								...
								while ($i < $num) {
									...
									$gymid = $row_data['GYM_TYPE_ID'];
									$gym_detail = $row_data['GYM_TYPE_TITLE'];
									echo '<option value="' . $gymid . '">' . $gym_detail . '</option>';
									$i++;
								}
								?>
				...
				<table width="945">
					<tr>
						...
						<td width="164">
							<select id="gymer" name="gymer">
								<option value="">Select the option</option>
								<?php if ($row['GYM_FLAG'] == "Y") { ?>
									<option value="Y" selected="selected">Yes</option>
									<option value="N">No</option>
								<?php } else if ($row['GYM_FLAG'] == "N") { ?>
									<option value="Y">Yes</option>
				...
				<table>
					<tr>
						...
						<td><input name="new_member" type="button" value="New Member" style="background:#91400F; height:30px; width:115px; font-weight:700;" onclick="window.location.href='gym_reg_form.php'" /></td>
						<td width="116"><input name="print" type="button" value="Print" style="background:#91400F; height:30px; width:115px; font-weight:700;" onclick="submitMe(this);" /></td>
						<td width="116"><input name="search" type="button" value="Search" style="background:#91400F; height:30px; width:115px; font-weight:700;" onclick="search_gym_member();" /></td>
						<td width="518"><input name="exit" type="button" value="Exit" style="background:#91400F; height:30px; width:115px; font-weight:700;" onclick="window.location.href='gym_admin.php'" />
						</td>
