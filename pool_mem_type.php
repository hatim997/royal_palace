...
if (isset($_REQUEST['add'])) {
	...
	$query0 = "INSERT INTO pool_membership_period_tab (MEMBER_PERIOD_ID, MEMBER_PERIOD_TITLE, CREATED_ID, CREATED_ON) 
	...
	$query0 = "UPDATE pool_membership_period_tab SET MEMBER_PERIOD_ID = '$mem_id', MEMBER_PERIOD_TITLE = '$mem_type', EDITED_ID = '$edited_id',
	EDITED_ON = '$edited_on' WHERE MEMBER_PERIOD_ID = '$mem_id'";
	$result0 = mysqli_query($conn, $query0) or die("error" . mysqli_error($conn));

	$sql1 = "SELECT * FROM pool_membership_period_tab WHERE MEMBER_PERIOD_ID = '$mem_id'";
	...
	$query0 = "DELETE FROM pool_membership_period_tab WHERE MEMBER_PERIOD_ID = '$mem_id'";
	$result0 = mysqli_query($conn, $query0) or die("error" . mysqli_error($conn));
	...
	foreach ($_REQUEST['radiobox'] as $m_id) {
		$sql1 = "SELECT * FROM pool_membership_period_tab WHERE MEMBER_PERIOD_ID = '$m_id'";
		$result1 = mysqli_query($conn, $sql1) or die("error" . mysqli_error($conn));
	}
	$row = mysqli_fetch_array($result1);
...
<head>
	...
	<script type="text/javascript">
		function search_mem_type() {
			...
			xmlhttp.open("POST", "search_mem_type.php", true);
			xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
			xmlhttp.send();
		}
	</script>
	<script type="text/javascript">
		function search_m_type() {
			...
			xmlhttp.open("POST", "ajax/search_m_type.php", true);
			xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
...
<body id="main_body">
	...
	<div id="form_container">
		...
		<form id="form1" class="appnitro" method="post" action="" onsubmit="return validateForm(); ">
			...
			<?php
			$query2 = "SELECT * FROM pool_membership_period_tab WHERE MEMBER_PERIOD_ID = '$m_num'";
			$result2 = mysqli_query($conn, $query2) or die('Query Failed00');

			if (mysqli_num_rows($result2) == 0) {
				$query3 = "SELECT MAX(MEMBER_PERIOD_ID) AS max_id FROM pool_membership_period_tab";
			...
			?>
				...
				<table>
					<tr>
						...
						<td width="116"><input name="search" type="button" value="Search" style="background:#91400F; height:30px; width:115px; font-weight:700;" onclick="search_mem_type();" /></td>
