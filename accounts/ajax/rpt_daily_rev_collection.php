...
<?php
	...
	if($rest != "A")
	{
		...
		<tr>
			...
			<td width="300" align="left">rpt_daily_rev_collection.php</td>
		</tr></table><br>';
		
	$query = "SELECT * FROM restaurant_master_tab WHERE REST_ID = '$rest'";
	$result = mysql_query($query);
	...
	while($a < $num1)
	{
		...
		echo '<div id="del_dish" align="center" style="overflow:auto; height:450px; width:960px">
				<table width="900" border="1" cellpadding="0" cellspacing="0" align="center">
				  ...
				  <tr>
					  ...
					  <td width="250" align="center"><strong>Restaurant</strong></td>
					  <td width="100" align="center"><strong>No Of Orders</strong></td>
		...
		while($from < $to)
		{
			...
			$query = "SELECT order_master_tab.ORDER_NO,order_master_tab.TOTAL_CHARGES,
						...
						order_master_tab.PAYMENT_MODE,order_master_tab.PAYMENT_REC,
						tax_charges_history.TAX_VALUE
						FROM order_master_tab, tax_charges_history
						WHERE order_master_tab.REST_ID = '$rid'
			...
			echo '<tr>
				  <td width="90" align="left">&nbsp;'.$from.'</td>
				  <td width="150" align="left">&nbsp;'.$rname.'</td>
		...
		$a++;
			...
			<tr>
				  ...
				  <td width="80" align="left">&nbsp;'.$gtotal_serv.'</td>
	...
	$query = "SELECT * FROM restaurant_master_tab ORDER BY REST_ID ASC";
	$result = mysql_query($query);
	$num1 = mysql_numrows($result);
	//mysql_close();
	//$num1--;
	...
	while($a < $num1)
	{
		...
		while($fro < $to)
		{
			...
			$total_discount = 0;
			$net_rec = 0;
			$fro = date("Y-m-d", mktime(0, 0, 0, $st_date[1], $st_date[2]+$d, $st_date[0]));
			$start = $from." 00:00:00";
			$end = $from." 23:59:59";
			$query = "SELECT order_master_tab.ORDER_NO,order_master_tab.TOTAL_CHARGES,
						order_master_tab.TOTAL_DISCOUNT,order_master_tab.TOTAL_SERV_CHARGES,
						...
						WHERE order_master_tab.REST_ID = '$rid'
						AND order_master_tab.TAX_ID = tax_charges_history.TAX_ID 
						AND order_master_tab.VISITED_DATE = '$fro' 
						ORDER BY order_master_tab.ORDER_NO ASC";
										
			$result1 = mysql_query($query) or die ('Query failed!'.mysql_error());
			$num = mysql_numrows($result1);
			//echo "Id is ".$rid." and num is ".$num;
			$i=0;
			while($i < $num)
			{
				$on = mysql_result($result1,$i, "order_master_tab.ORDER_NO" );
